...
int main(int argc, char **argv)
{
    ...
    if (opt.allNodes && !opt.nb_areas)
    {
        ...
        Best2cop(&pfront, &pf, sr, 0, opt.cstr1, opt.cstr2, max_dict_size, opt.analyse, NULL, opt.bascule, NULL);
        for (int j = 0; j <= 10; j++)
            {
    ...
    else if (opt.src != -1 || opt.src_lab != NULL)
    {
        ...
        Best2cop(&pfront, &pf, sr, opt.src, opt.cstr1, opt.cstr2, max_dict_size + 1, opt.analyse, NULL, opt.bascule, NULL);
        pf = NULL;
        pfront = NULL;
        int init_time = 0;
        ...
        if (!opt.analyse && !opt.allNodes)
        {
            ...
            for (int i = 0; i < opt.nb_areas; i++)
            {
                for (int id = 0; id < 2; id++)
                {
                    ...
                    iter = Best2cop(&pfront, &pf, sr_areas[i], area_src, opt.cstr1, opt.cstr2, max_dict_size + 1, false, NULL, opt.bascule, &init_time);
                    ...
                    gettimeofday(&stop, NULL);
                    times_area[index] = (stop.tv_sec - start.tv_sec) * 1000000 + stop.tv_usec - start.tv_usec - init_time;
...
***********************************************************************************/
    ...
    for (int i = 0; i < opt.nb_areas; i++)
    {
        for (int id = 0; id < 2; id++)
        {
            ...
            /* first do the BEST2COP on each areas */
            //INFO("Start best2cop\n");
            iter = Best2cop(&pfront, &pf, sr_areas[i], area_src, opt.cstr1, opt.cstr2, 1001, opt.analyse, NULL, opt.bascule, NULL);
            //INFO("End BEST2cop\n");
            /* then retreive the paths using the pareto front */
            //INFO("Start Retreive\n");
            sl_areas[index] = Segment_list_retreive_paths(pf, sr_areas[i], iter, area_src);
    ...
    iter = Best2cop(&pfront, &pf, sr_areas[opt.nb_areas - 1], src, opt.cstr1, opt.cstr2, 1001, false, NULL, opt.bascule, NULL);
    ...
    for (int i = 1; i < opt.nb_areas; i++)
    {
        for (int id = 0; id < 2; id++)
        {
            ...
            else
            {
                ...
                gettimeofday(&start, NULL);
                /* first do the BEST2COP on each areas */
                iter = Best2cop(&pfront, &pf, sr_areas[0], area_src, opt.cstr1, opt.cstr2, max_dict_size + 1, false, NULL, opt.bascule, &init_time);
