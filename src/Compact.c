...
int **get_nb_paths_per_segments(Dict_t **dist, int nbNodes, int iter)
{
	int **nb_paths = calloc(nbNodes, sizeof(int *));
	for (int i = 0; i < nbNodes; i++)
	{
...
// number of segments
compact_front compact_to_array_2D(__attribute__((unused)) Pfront_t **pf, Dict_t **dist, int iter, int nbNodes, struct segment_list ***sl)
{
	path ***compact_pf;
	...
	for (int i = 0; i < nbNodes; i++)
	{
		compact_pf[i] = calloc(iter + 1, sizeof(path *));
		compact_pf[i][iter] = NULL;
	...
	compact_front cf = {.paths = compact_pf, .nbNodes = nbNodes};
	return cf;
}
...
struct segment_list merge_and_correct_sl(struct segment_list sl1, struct segment_list sl2)
{
	...
	{
		sl3.seg[i] = sl2.seg[i];
		sl3.seg_type[i] = sl2.seg_type[i];
	}

	return sl3;
}

Dict_t **cart(compact_front pf1, compact_front pf2, int c1)
{
	// pf1 = dist to ABR
	Dict_t **pf3 = NULL;
	for (int i = 0 ; i <= 10 ; i++) {
		...
		for (int d1_index = 0; pf1.paths[ABR][s1_index][d1_index].cost != -1; d1_index++)
		{
			...
			for (int out_node = 0; out_node < pf2.nbNodes; out_node++)
			{
				...
				for (int s2_index = 0; pf2.paths[out_node][s2_index]; s2_index++)
				{
					...
					{
						int delay2 = pf2.paths[out_node][d2_index][s2_index].delay;
						...
						struct segment_list sl3 =
							merge_and_correct_sl(pf1.paths[ABR][d1_index][s1_index].sl,
						...
						int cost3 = pf2.paths[ABR][d1_index][s1_index].cost
							+ pf2.paths[out_node][d2_index][s2_index].cost;
