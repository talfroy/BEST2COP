...
{
    if (prev == NULL) {
        return Edge_new(NULL, m1, m2, segType);
    }
    ...
    prev->next = Edge_add(prev->next, m1, m2, segType);
    return prev;
}
...
Edge_t* Edge_copy(Edge_t* old)
{
    ...
    return Edge_new(last, old->m1, old->m2, old->seg_type);
}
...
Edge_t* Edge_merge_flex(Edge_t* best_m1, Edge_t* best_m2)
{
    ...
    flex = Edge_new(NULL, best_m1->m1, best_m1->m2, best_m1->seg_type);
    if (flex->m1 > best_m2->m1) {
        if (flex->m2 >= best_m2->m2) {
            Edge_free(flex);
            flex = NULL;
        }
        return Edge_new(flex, best_m2->m1, best_m2->m2, best_m2->seg_type);
        ...
        if (flex->m2 > best_m2->m2) {
            Edge_free(flex);
            flex = Edge_new(NULL, best_m2->m1, best_m2->m2, best_m2->seg_type);
        }
    }
    return flex;
}